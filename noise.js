class Noise {
    ...
    noise(x, y, z) {
        ...
        let X = parseInt(Math.floor(x)) & 255;
        let Y = parseInt(Math.floor(y)) & 255;
        let Z = parseInt(Math.floor(z)) & 255;
        x -= Math.floor(x);                                // FIND RELATIVE X,Y,Z
        y -= Math.floor(y);                                // OF POINT IN CUBE.
        z -= Math.floor(z);
        let u = this.fade(x);                                // COMPUTE FADE CURVES
        let v = this.fade(y);                                // FOR EACH OF X,Y,Z.
        let w = this.fade(z);
        
        let A = this.p[X  ]+Y, AA = this.p[A]+Z, AB = this.p[A+1]+Z;      // HASH COORDINATES OF
        let B = this.p[X+1]+Y, BA = this.p[B]+Z, BB = this.p[B+1]+Z;      // THE 8 CUBE CORNERS,
